...
export function initPWA(onSharedFilesReceived) {
  ...
  if (installBtn) {
    installBtn.addEventListener('click', async () => {
      ...
      console.log('[Encre] Install prompt outcome:', outcome);
      deferredPrompt = null;
  ...
  window.addEventListener('appinstalled', () => {
    console.log('[Encre] PWA successfully installed!');
  ...
  try {
    ...
    for (const req of requests) {
      ...
      if (response) {
        ...
        const rawName = response.headers.get('x-file-name') || 'shared-image';
    ...
    console.warn('[Encre] Error reading shared files:', err);
  }
}
